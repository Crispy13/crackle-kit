...
fn bench_range_access(c: &mut Criterion) {
    let sequences = generate_sequences(NUM_SEQS, SEQ_LEN);
    ...
    let access_ranges: Vec<(usize, std::ops::Range<usize>)> = (0..1000)
        .map(|_| {
            let start = rng.random_range(0..SEQ_LEN - 1);
            let end = (start + rng.random_range(1..SEQ_LEN)).min(SEQ_LEN);
    ...
    group.bench_function("Vec<String>", |b| {
        b.iter(|| {
            for &(seq_idx, ref range) in &access_ranges {
                ...
                for c in string_slice.as_bytes() {
                    black_box(c);
                }
            }
        })
